 ...
 *   26 文字の行（`ROW_LETTERS`）は**削除した**。⚠ あれは冗長だったのではなく
 *   **範囲が実データより広い現役の定数**で、残すと**画面に出ない値が検証だけを通る**
 *   （`D1` は選べないのに保存はできる、という状態になる）。
 *   → **選択肢と検証の範囲は、同じ 1 つの真実から引く。**
 */
...
export function parseCoordinate(text: string): Record<string, unknown> {
  if (!COORDINATE_CHOICES.includes(text)) {
    // ⚠⚠ ここを `{}` にしても**観測できる違いは無い**——実測で変異を 2 通り当てて
    //   どちらも **0 赤**（3 述語はどちらも「空」と判定し、画面の表示も同じ）。
    //   ⭐ **これは「検査が足りない」ではなく「観測上ほんとうに等価」である。**
    //   → **述語を足さない。** 足すと*誰も依存していない内部の形*を固定することになり、
    //     **壊れたときに利用者に何が起きるかを言えないテスト**が 1 本増える。
    //   → 代わりに**なぜこの形で返すか**を残す: 下書きの形を `createDraft()` と一致させるため
 ...
 */
/**
 * ⚠⚠ **入力欄としてはもう使わない**（3×3 の 9 択 1 つになった・§1-3-3d ①）が、
 *   **これは冗長な宣言ではない。現役である。**
 *
 * ⭐ 保存形（`{row, col}`）に対する**検証・刈り取り・打ちかけ判定の 3 つとも、
 *   いまもこの 2 フィールドの上に乗っている**（`integer` の整数チェックもここが担う）。
 *   **実測（2026-08-24）**: この配列を空にする変異で **25 本が赤**になる。
 *   ⚠ 消すと、座標が 3 述語を**素通り**する（例外は出ない）。
 *
 * ⚠ **「入力欄に出ない」＝「使われていない」ではない。**
 *   画面から辿れないコードは消してよい、という読み方でここへ来た人のために書いておく。
 */
