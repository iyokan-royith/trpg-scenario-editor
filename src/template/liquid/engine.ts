 ...
 *   値はすべて**ロイスの決定**であり、実装側で選んだものは 1 つも無い
 *   （出典は §1-13-1c だが、`lenientIf` だけは後から来た §1-13-1g）:
 *
 *   | 設定 | 値 | 出典 | 決定の理由 |
 *   |---|---|---|---|
 *   | `strictVariables` | `true` | §1-13-1c | 既定 `false` だと**未定義変数が黙って空文字**になる（実測） |
 *   | `strictFilters`   | `true` | §1-13-1c | 既定 `false` だと**存在しないフィルタが黙って無視**される（実測） |
 *   | `renderLimit`     | `10000` | §1-13-1c | 「いったん10秒ぐらいにして」。⚠ **これが無いと `{% render %}` の自己参照が本物の無限ループになる** |
 *   | `outputEscape`    | HTML 側だけ `'escape'` | §1-13-1c | **エンジン単位でしか切り替えられない**ので 2 インスタンスに分ける |
 *   | **`lenientIf`**   | `true` | **§1-13-1g** | ロイス決定「**フラグは立てましょう**」。`strictVariables: true` のままだと**省略可フィールドを守る手段が言語側に無い**（`{% if %}`・`{% unless %}`・`default` フィルタが**どれもガードにならない**＝実測）。⚠ **代償は下記** |
 *
 * ⚠⚠ **`lenientIf: true` で何を失い、何が残ったか**（§1-13-1g の実測表。
 *   `__tests__/engine.spec.ts` に**4 本とも述語として固定してある**——
 *   ここを消したときに鳴るのはその述語だけなので、消す前に読むこと）:
 *
 *   | | |
 *   |---|---|
 *   | **失った保護** | `{% if descrption %}` — **条件の中の誤字が黙って通る**（「フィールドが無い」と見分けが付かない） |
 *   | ⭐ **残った保護** | `{{ descrption }}` — **出力の誤字は ERR のまま**。§1-13-1c の主眼（**黙って空文字にしない**）はここで守られている |
 *
 * ⚠ **「テンプレの瑕疵はテンプレ側で直す」という §1-13-1g のロイス決定と対になっている。**
 *   守る手段があって初めて「守らないのはテンプレの瑕疵」と言える——
 *   だから**このフラグを外すなら、同梱テンプレの `{% if %}` ガードも同時に意味を失う**。
 *
...
export function liquidOptionsFor(purpose: LiquidPurpose): LiquidOptions {
  return {
    ...
    strictFilters: true,
    // ⚠ §1-13-1g の決定。`strictVariables` を緩めるのではなく、**`{% if %}` の中だけ**緩める。
    //   出力 `{{ }}` の誤字は鳴ったまま（上の表）。
    lenientIf: true,
