 ...
 *   ⚠ **原本に無い値は足していない**——`roomStats` を持つ部屋は 2 室（`room-2`＝B3・`room-3`＝A2）で、
 *   どちらも spike の調査で確定した実際の表示値（B3: T1/E10・A2: T2/E0）をそのまま入れている。
 *   ⚠ **A2 の `enemyCount` は【ものかげ】による偽装**（隠れている分を計上しない効果）で、
 *   `reason` にその理由が入っている——**表示は他の値と同じ形式だが、データ上は区別できる**。
 *   他の 7 室が `roomStats` を持たないのも元データがそうだから（宣言側の既定値で T0/E0 に揃える）。
 *
